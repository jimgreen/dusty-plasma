...
vzc_e1 = jzc_e /qe/(nc)
vzc_i1 = jzc_i /qi/(nc)
vzc_e2 = jzc_e /qe/(0.5*(n1+n0))
vzc_i2 = jzc_i /qi/(0.5*(n1+n0))
...
log("vzc_e1 = %g = %g vA0", vzc_e1, vzc_e1/vA0)
log("vzc_i1 = %g = %g vA0", vzc_i1, vzc_i1/vA0)
log("vzc_e2 = %g = %g vA0", vzc_e2, vzc_e2/vA0)
log("vzc_i2 = %g = %g vA0", vzc_i2, vzc_i2/vA0)
...
init = function(x,y,z)
   ...
   local n = nh+nb
   n = n * (1 + Nnoise_level*math.random()*math.random(-1,1))
   local rho_e = n*me
   local rho_i = n*mi
   ...
   pxx_e = pxx_e + p_e * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pyy_e = pyy_e + p_e * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pzz_e = pzz_e + p_e * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pxy_e = pxy_e + p_e * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pxz_e = pxz_e + p_e * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pyz_e = pyz_e + p_e * (1 + Pnoise_level*math.random()*math.random(-1,1))
   local p_i = p - p_e
   local pxx_i = p_i
   local pyy_i = p_i
   local pzz_i = p_i + rhovz_i^2/rho_i
   local pxy_i, pxz_i, pyz_i = 0,0,0
   pxx_i = pxx_i + p_i * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pyy_i = pyy_i + p_i * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pzz_i = pzz_i + p_i * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pxy_i = pxy_i + p_i * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pxz_i = pxz_i + p_i * (1 + Pnoise_level*math.random()*math.random(-1,1))
   pyz_i = pyz_i + p_i * (1 + Pnoise_level*math.random()*math.random(-1,1))

   local vz = (rhovz_e + rhovz_i)/(rho_e+rho_i)
   local Ex = 0
   local Ey = -vz*Bxb
   local Ez = 0
...
----------------------------
decomposition = DecompRegionCalc2D.CartProd { cuts = {Lucee.getNumProcs(), 1} }
...
elcCollisionlessSrcSlvr = Updater.PeriodicCollisionlessHeatFlux2D {
   onGrid = grid,
   closure = "global",
}

ionCollisionlessSrcSlvr = Updater.PeriodicCollisionlessHeatFlux2D {
   onGrid = grid,
   closure = "global",
}
