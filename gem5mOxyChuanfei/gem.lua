...
B0 = lightSpeed * math.sqrt(mu0 * n0 * me) / wpe0_wce0
...
Te0 = p0 * pe_frac / n0 -- p0 * pe_frac / (n0 * ne_frac)
Ti0 = p0 * pi_frac / n0 -- p0 * pi_frac / (n0 * ni_frac)
To0 = Ti0 -- p0 * po_frac / (n0 * no_frac)

wci0 = qi*B0/mi
-- hydrogen plasma frequency based on number density n0*ni_frac
wpi0 = math.sqrt(n0 * qi^2 / epsilon0 / mi)
...
vzc_o = jzc_o /qo/(n0+nb) -- jzc_i /qo/(n0+nb)
...
init = function(x,y,z)
   ...
   -- local n_e = n * ne_frac
   local n_i = n -- n * ni_frac
   local n_o = n0 * no_frac -- n * no_frac
   local n_e = n_i + n_o
   local rho_e = n_e*me
   local rho_i = n_i*mi
   local rho_o = n_o*mo
